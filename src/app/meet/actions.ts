...
export async function submitDemoRequest(
  ...
  //
  //    Die Herkunft wird VOR der Validierung normalisiert, weil sie deren
  //    Ergebnis mitbestimmt: Auf /research ist das Nachrichtenfeld die
  //    Forschungsfrage und damit Pflicht. Der Wert bleibt Client-Eingabe und
  //    faellt bei allem Unbekannten still auf „demo" zurueck.
  ...
  const result = validateDemoRequest({
    ...
    research: Object.fromEntries(
      RESEARCH_FIELDS.map((feld) => [feld, readString(formData, feld)]),
    ),
  });
  ...
  //    Anfrage laengst angenommen – und der Besucher las trotzdem „could
  //    not be submitted" und schickte ein zweites Mal. Wir hatten die
  //    Anfrage doppelt, er hatte das Gefuehl, ins Leere zu schreiben.
  //
  ...
  //    geloggt – Kanal, Zustand, secret-frei –, und zwar auch dann, wenn der
  //    andere getragen hat. Ein Teil-Ausfall, den niemand sieht, waere in
  //    einer Woche ein Total-Ausfall, den alle sehen.
  //
  ...
  // Formular – keinen Namen, keine Adresse, keine Zieladresse, keinen
  ...
  // betreffen: „not set up" ist unser Konfigurationsfehler und darf niemanden
  // zum Wiederholen einladen; alles andere kann ein Aussetzer sein, bei dem
  // ein zweiter Versuch sinnvoll ist.
  //
  // DIE ZWEITE MELDUNG NENNT EINEN WEG, DER NICHT UEBER UNS LAEUFT. Wenn
  // beide Kanaele liegen, ist ein „bitte erneut versuchen" allein eine
  // Zumutung – die Adresse steht deshalb daneben.
  const beideNichtEingerichtet =
    mail.reason === "not-configured" && crm.reason === "not-configured";

  return {
    status: "error",
    message: beideNichtEingerichtet
      ? "Sending is not set up at the moment. Your request was not submitted."
      : `Your request could not be submitted just now. Please try again. You can also reach us at ${imprint.email}.`,
  };
}
